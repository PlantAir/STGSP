...
class STGSP(nn.Module):
    def __init__(self, data_conf):
        ...
        self.extnn_inter = nn.Sequential(
            ...
            nn.SELU(),
            MoKLayer(self.mconf.inter_extnn_inter_channels, self.mconf.res_nbfilter*self.dconf.dim_h*self.dconf.dim_w),
            nn.SELU()
        )
        ...
        self.extnn_last = nn.Sequential(
            ...
            nn.SELU(),
            MoKLayer(self.mconf.last_extnn_inter_channels, self.dconf.dim_flow*self.dconf.dim_h*self.dconf.dim_w),
            nn.SELU()
        )
